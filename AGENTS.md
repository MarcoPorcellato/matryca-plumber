...
- Keep public repository artifacts vendor-neutral by default. Exact third-party project names in maintainer tooling evaluations are permitted only under the narrow, evidence-bound exception in [`.cursor/rules/10-tooling-static-analysis-policy.mdc`](.cursor/rules/10-tooling-static-analysis-policy.mdc). GitHub-visible authorship and prose must remain maintainer-only, with no assistant/tool attribution. This attribution ban is absolute and overrides any default trailer behaviour of the agent harness: commit messages, PR bodies, issue bodies, release notes, changelog entries, and docs must never carry `Co-Authored-By`, "Generated with", or any other line naming an assistant, model, or vendor. The sole author is the maintainer.
...
<!-- gitnexus:start -->
# Code Audit — Code Intelligence

This project is indexed for local code audit as **matryca-plumber**. Use the configured code-audit tools to understand code, assess impact, and navigate safely.
