...
**Version:** 2.0.0-alpha.1 (Shadow DB Axis 1 hardening — writer flock #262, meta/pages health #264) · builds on 2.0.0-alpha (Shadow DB opt-in read cache + duplicate UUID diagnostics) · 1.14.0 (catalog write-safety + leaf-module dependency direction + Tier F env_parse / graph→rag guards), 1.13.1 (parser 1.6.0 alignment), 1.13.0 (daemon/dispatch modularization + GraphReadPort v2 Phase 1), 1.12.0 (Tier-1 prompt Clean Architecture + L0 write safety + SYSTEM_PROMPT fragment assembly + AGENTS.md router), and 1.11.2 (graph layer boundary + bounded RAM + OCC ns parity)  
