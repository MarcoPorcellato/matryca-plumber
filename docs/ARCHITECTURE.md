...
**v2.0.0-alpha.1 focus:** **Shadow DB Axis 1 hardening** — cross-process writer coordination via advisory `shadow.writer.flock` ([#262](https://github.com/MarcoPorcellato/matryca-plumber/issues/262)); meta/pages consistency gate before `ready` health ([#264](https://github.com/MarcoPorcellato/matryca-plumber/issues/264)); audit tracker [#261](https://github.com/MarcoPorcellato/matryca-plumber/issues/261). Builds on alpha read routing below.
...
The UI never becomes a second source of truth: it reads daemon checkpoints and live graph scans; configuration writes go to the repo **`.env`** atomically and are picked up by `reload_plumber_dotenv()` on the next daemon sync cycle. Cooperating UI requests serialize the complete read/merge/write transaction. Each transaction binds the source to device, inode, nanosecond mtime, size, and SHA-256, then rechecks that identity after the temporary file is flushed and immediately before `os.replace`; an intervening external edit returns the content-free HTTP 409 code `dotenv_conflict` instead of being overwritten. Comments, unknown keys, and established ordering survive the merge, and process-environment reload occurs only after the replacement commits.
