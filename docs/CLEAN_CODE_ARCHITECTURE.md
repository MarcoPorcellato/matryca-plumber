...
| `maintenance_daemon` SRP | ~~3300 lines~~ **~1500** after #58 slices; bootstrap + `run_cycle` orchestration remain | [#58](https://github.com/MarcoPorcellato/matryca-plumber/issues/58) — slices: `daemon_state`, `daemon_process_lock`, `daemon_semantic_write`, `daemon_page_queue`, `daemon_llm_cycle` |
