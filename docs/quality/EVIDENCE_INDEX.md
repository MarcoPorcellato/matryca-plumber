...
| `LOGSEQ-DB-CLI-SCHEMA-001` | The separately admitted bundled CLI has a hash-bound shallow JSON result envelope and Gate B observation shapes for graph, property, page/root show, and server-list commands. | Exact macOS arm64 DMG inspected 2026-09-15 | `docs/quality/LOGSEQ_DB_CLI_STATIC_SCHEMA_EVIDENCE_2026-09-15.md` | DMG `1a71e4c0…cef429f`; `app.asar` `9c476c01…7799ca7`; embedded CLI `b3710e5a…eb40a98`. | `partial` | Static evidence only: it does not execute Logseq, prove a fixture or read, permit `server stop`, establish a terminal transport result, or support a Plumber DB adapter. | 2026-12-14 |
| `LOGSEQ-DB-CLI-OBSERVER-001` | A proposed private observer design binds the pre-launch validation, root containment, bounded capture, lifecycle inventory, and stop rules required before a recovery observation can execute. | Proposed Gate B recovery preparation | `docs/quality/LOGSEQ_DB_CLI_RECOVERY_OBSERVER_SAFETY_DESIGN_2026-09-16.md` | Exact admitted DMG, `app.asar`, and embedded CLI digests; fixed command and root policy. | `proposed` | It is non-executing design only. It proves no artifact admission, fixture, host process, graph read, lifecycle behavior, transport result, or product support. | 2026-12-15 |
| `LOGSEQ-DB-CLI-DB0-001` | The bundled Logseq CLI lane is upstream-blocked before DB-0 admission because documented CLI startup can manage workers and lock state while upstream supplies no immutable signed read-observer/provenance contract. | Official CLI public documentation reviewed 2026-09-19 | `docs/quality/LOGSEQ_DB_CLI_DB0_ADMISSION_BLOCKER_2026-09-19.md` | Official CLI, DB-version, plugin, nightly-release, and issue references recorded in the bounded evidence record. | `blocked` | No artifact, fixture, graph, database, or executable was opened; this blocks only the CLI lane and does not classify Plugin SDK or MCP. | 2026-12-18 |
...
| `V2-0-1-RC4-QUAL-001` | The proposed RC4 plan classifies the complete delta as Tier 3 and requires candidate-bound hosted CI, installed-package parity/TCK, targeted controls, and fresh dual-profile Gate B. | Future v2.0.1-rc.4 candidate | `docs/quality/V2_0_1_RC4_RELEASE_QUALIFICATION_PLAN_2026-09-06.md`; `docs/quality/RISK_BASED_RELEASE_QUALIFICATION_DECISION_2026-08-24.md` | Planning anchor `118b265b5c6b29682c76453aad5fbde0de0c841f`; #582; delta table covers Parser 1.9 snapshot, timeout lifecycle, topology session, static resources, test-only DB policy, and #580 blocked evidence. | `proposed` | No candidate, tag, hosted CI run, artifact hash, installed TCK/parity result, Gate B campaign, publication, DB support, or stable decision is selected. Each Tier 3 Gate B profile needs 259,200 valid seconds against the exact public artifact. | 2027-03-06 |
