...
| `CCP-HYBRID-CONTRACT-001` | The repository defines a pinned multi-runtime local-CI contract, opt-in trusted exact-head receipt observer, immutable savings ledger, and fail-closed operating path without changing hosted-CI authority. | Task 6 Step 0 accepted source boundary | `.commit-ci-preflight.toml`; `.commit-ci-policy.toml`; `.github/workflows/receipt-gate.yml`; `docs/quality/CCP_HYBRID_CI_RUNBOOK.md` | Deterministic contract tests and plan-derived policy bindings cover three runtimes and 18 required checks, including source security; ineligible or unlabelled PRs publish no receipt status and hosted `.github/workflows/ci.yml` remains unconditional. | `proposed` | No official matrix receipt exists. Coordinator diagnosis, OCI architecture evidence, parity, seven negative receipt cases, routing activation, ruleset changes, and claimed savings remain blocked by their independent gates. | 2026-11-22 |
