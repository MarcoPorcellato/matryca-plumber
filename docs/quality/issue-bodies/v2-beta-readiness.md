...
| Defect threshold | No open P0/P1 defect in beta scope; every remaining P2 has an explicit disposition | [ ] — **needs maintainer input.** The repository carries no priority labels, so this cannot be derived mechanically: 46 issues are open and none is marked as a beta-scope blocker. The disposition set must be supplied to the readiness harness by the maintainer |
