...
| Upgrade and rollback safety | Clean install plus `1.14.5`, `2.0.0a5`, and `2.0.0b1` upgrades to the exact RC candidate; schema mismatch, failed rebuild, opt-out, and rollback/recovery fall back without partial reads or Markdown writes | [ ] — run `scripts/qualify_rc_upgrade_rollback.py` against the frozen local RC wheel with all three `--baseline` values, an explicit source-vault realpath fingerprint, and a disposable output root; record only its sanitized terminal result. |
...
- `scripts/qualify_rc_upgrade_rollback.py` — fail-closed installed-wheel RC
  upgrade, default-on/read-only, and rollback qualification
