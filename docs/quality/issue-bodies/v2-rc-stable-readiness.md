...
| Cross-platform gate | Linux, macOS, and Windows CI or installed-runtime evidence passes for the supported Shadow read contract | [ ] — current `main@ecd4f854` has successful Ironclad, macOS Shadow-contract, Windows Shadow-contract, and Python 3.13 jobs; the final stable candidate must repeat this receipt after the version/documentation changes. |
...
| Stable documentation | In-memory BM25 is deprecated as the default discovery path but remains a supported fallback; no beta/RC wording survives on stable surfaces | [ ] — being finalized with the exact stable candidate across README-adjacent agent surfaces, the Shadow operator contract, OpenSpec onboarding, and generated system prompt. |
