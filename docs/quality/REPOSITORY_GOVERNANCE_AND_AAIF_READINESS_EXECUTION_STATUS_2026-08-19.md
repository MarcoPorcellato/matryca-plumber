...
| M4 — outcome evaluation | `in_progress` | reproducible graph-outcome evidence | retain #527's synthetic corruption evidence, then add production-adapter scenarios; do not infer agent, release, or platform qualification |
| M5 — CI, release, and recovery | `in_progress` | resource-admission and recovery receipts | obtain separate review and merge authorization for CCP PR #49, then install its telemetry change and validate interruption, reboot, stale-checkpoint, disk-pressure, and service-restart recovery before claiming an operator-path exit gate |
...
| 2026-08-19 | Graph-outcome corruption slice planned | Issue #527 is the non-duplicative child slice under #483 for a provider-free corrupt derived-state failure-injection profile, explicit safe no-serve/recovery result, canonical immutability, and reset-isolation proof. It is planned work, not evidence of an implemented scenario or runtime recovery behavior | `planned` |
| 2026-08-19 | CCP terminal-detail hosted qualification | CCP PR #49 head `c7a88bde9ab67b0102ce323b7fc1766e0b23673d` first failed closed because its evidence branch was absent. After macOS `macos-v4` admission at 71% available memory and zero swap, the exact pinned container contract completed all five checks. Receipt SHA-256 `da5b0c8af2a58b9ffbb65f33f54cc9a57328156bfcce9c444d615d3413701b3c` passed integrity and policy with a verifier built from trusted base `73ec2a32f730b075cd95f59e084e0ce80de609e9`, then was published without force-push to `ccp-evidence/c7a88bde9ab67b0102ce323b7fc1766e0b23673d`. The rerun GitHub gate completed PASS. The PR remains unmerged and its terminal-detail telemetry is not yet installed | `verified` |
| 2026-08-19 | Corrupt derived-state synthetic evidence | On clean stacked head `aca35b77eefde2b649c2645c31149cd1bb1163f1`, issue #527 added a corrupt-derived-state profile that rejects retrieval with a no-serve outcome, preserves canonical bytes, and proves the following Strict Read Only episode receives fresh roots. The new external content-free report has SHA-256 `b6f7a0a78d13a9cd607567a18001b5714b748eada63a964e17b3fb9693cc5239`; all eight declared synthetic checks completed. Focused tests completed `16 passed`; a resource-guarded pre-commit `make ci` also completed without a watchdog trip. This is source-bound synthetic infrastructure evidence only, not a production adapter, real Shadow recovery, agent, external system, platform, soak, or release qualification | `verified` |
