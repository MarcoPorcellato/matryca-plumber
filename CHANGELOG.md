...
- **Shadow DB: pages over the parse budget are excluded from the read cache.** A full-corpus measurement on a sanitized 3,378-page daily-use vault copy found **25 pages (0.74%)** whose parse exceeds the 15 s default, at 41.65–58.33 s each, accounting for **90.3% of total parse time**; parse cost is bimodal and uncorrelated with page size (the largest page, 650,106 B, parses in 0.54 s). Those pages are now quarantined rather than aborting the rebuild — see the per-page quarantine entry under Added — so the rest of the graph is cached and reads for parked pages use Markdown, which stays authoritative. The residual limitation is that a quarantined page gets no FTS or subtree acceleration until it parses within budget. Measurement and TRIZ analysis: [`docs/quality/SHADOW_DB_PARSE_BUDGET_TRIZ_2026-07-27.md`](docs/quality/SHADOW_DB_PARSE_BUDGET_TRIZ_2026-07-27.md).
