...
- **`maintenance_daemon` SRP ([#58](https://github.com/MarcoPorcellato/matryca-plumber/issues/58))** — split orchestrator into `daemon_state`, `daemon_process_lock`, `daemon_semantic_write`, `daemon_page_queue`, `daemon_llm_cycle`, and `daemon_llm_client`; `maintenance_daemon.py` retains bootstrap, `run_cycle`, cluster grouping, and backward-compatible re-exports.
