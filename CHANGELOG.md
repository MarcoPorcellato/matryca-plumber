...
- **`graph_dispatch` read slice ([#59](https://github.com/MarcoPorcellato/matryca-plumber/issues/59), v2 Phase 0–1)** — extracted `dispatch_read_handlers.py` (one handler per `ReadGraphTarget`); `dispatch_read()` in `graph_dispatch.py` is now a thin delegate. Subtree reads route through `GraphReadPort` / `MarkdownGraphRepository`. Mutate/refactor/lint remain in `graph_dispatch.py` for follow-up slices. Documented in [`docs/CLEAN_CODE_ARCHITECTURE.md`](docs/CLEAN_CODE_ARCHITECTURE.md#graph-dispatch-module-map-issue-59).
- **`graph_dispatch` search slice ([#59](https://github.com/MarcoPorcellato/matryca-plumber/issues/59))** — extracted `dispatch_search_handlers.py` (one handler per `SearchGraphMethod`); `dispatch_search()` is a thin delegate. Tests extended in `tests/test_graph_dispatch_search.py`.
- **`graph_dispatch` mutate/refactor/lint slices ([#59](https://github.com/MarcoPorcellato/matryca-plumber/issues/59))** — extracted `dispatch_mutate_handlers.py`, `dispatch_refactor_handlers.py`, `dispatch_lint_handlers.py`. `graph_dispatch.py` keeps headless write runtime (`_headless_append_child`, outline writes) and thin `dispatch_*` delegates. Tests in `tests/test_graph_dispatch_mutate.py`.
