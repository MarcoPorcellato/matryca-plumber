...
- **Pocket Alpha 1A contract design and plan** — document the proposed closed,
  content-free, deterministic Plumber contract bundle and its evidence-gated
  implementation sequence for future signed Mobile Knowledge Pack producers
  and offline Android consumers; no runtime contract, pack generation, or
  Pocket behavior is implemented by this documentation-only change.
...
- **LLM-free cluster-recognition baseline** — document how deterministic BM25, Logseq tags/properties, wikilinks, graph generations, and content-free fingerprints can extend current TF-IDF/Louvain neighborhoods without making model inference a prerequisite; add a five-seed synthetic quality benchmark covering combined, summary-only, tag-only, and no-feature regimes.
- **LLM-free BM25 query-result cache** — resident in-process BM25 now keeps up to 8,192 immutable query results keyed by tokenized request, limit, and scoring parameters, with a second 65,536-result-row LRU budget for broad requests; per-corpus synchronization serializes scoring with batch graph changes before invalidation, and content-free capacity/hit/miss/invalidation counters support synthetic cold/warm measurement without adding a service or inference dependency.
...
- **Shadow DB: pages over the parse budget are excluded from the read cache.** On a sanitized daily-use vault copy, **3 of 1,014 graph pages (0.30%)** exceed the 15 s default parse budget and are parked. A filesystem sweep of the same vault finds 25 over-budget Markdown files out of 3,378, but 22 of those live in Logseq's own `version-files/` and `bak/` directories, which the cache never reads; the cache indexes `pages/` and `journals/` only. Parse cost is bimodal and uncorrelated with page size — the largest page, 650,106 B, parses in 0.54 s, while a 336,260 B page does not. Over-budget timings scale with machine load and have no bound this corpus can establish: one page did not finish within 115 s under ordinary interactive load. Those pages are quarantined rather than aborting the rebuild — see the per-page quarantine entry under Added — so the rest of the graph is cached and reads for parked pages use Markdown, which stays authoritative. The residual limitation is that a quarantined page gets no FTS or subtree acceleration until it parses within budget. Measurement and TRIZ analysis: [`docs/quality/SHADOW_DB_PARSE_BUDGET_TRIZ_2026-07-27.md`](docs/quality/SHADOW_DB_PARSE_BUDGET_TRIZ_2026-07-27.md); population reconciliation: [`docs/quality/SHADOW_DB_SOAK_24H_EVIDENCE_2026-07-28.md`](docs/quality/SHADOW_DB_SOAK_24H_EVIDENCE_2026-07-28.md).
...
- **RC upgrade/rollback qualification collector** — `scripts/qualify_rc_upgrade_rollback.py` verifies each declared published baseline in a fresh virtual environment, upgrades it to an explicit local RC wheel, checks default-on Shadow, explicit opt-out, Read Only external-cache isolation, schema-mismatch and failed-rebuild fallback/recovery, and Markdown/graph-file integrity, then reinstalls the baseline. Its result is path- and content-free, fail-closed, and bound to the supplied wheel digest.
- **Per-page Shadow DB parse quarantine** — a page whose bounded parse cannot complete within `MATRYCA_PAGE_PARSE_TIMEOUT_S` is now parked in a new `quarantined_pages` table instead of aborting the whole rebuild. The page is absent from `pages`, so every existing query, FTS trigger, and subtree CTE keeps its exact prior meaning and reads for it route to Markdown; the rest of the graph is cached and health stays `READY` under the widened invariant `indexed + quarantined == source == actual`. A page is released automatically as soon as it parses again. `GET /api/state` reports `shadow_db.quarantined_page_count`, which the Sovereign UI renders, and those pages are excluded from `lag_pages` because they are a settled decision rather than pending work. Quarantine rows are content-free — only the graph-relative file path already held in `pages.file_path`, plus size and timing counters. The schema addition is `CREATE TABLE IF NOT EXISTS`, so existing databases migrate on open. Set `MATRYCA_SHADOW_QUARANTINE_ENABLED=false` to restore the previous strict behaviour where any over-budget page fails the rebuild.
- **Shadow DB not-ready reason in `GET /api/state`** — `shadow_db.not_ready_reason` reports why the read cache is not accelerating reads, using a closed vocabulary of content-free codes (`not_bootstrapped`, `bootstrap_in_progress`, `database_unreadable`, `schema_version_mismatch`, `sync_error`, `full_sync_incomplete`, `page_count_mismatch`) and `null` when the state is `ready` or `disabled`. The Sovereign UI Shadow DB row renders it as a plain-language sentence. A bootstrap aborted by the per-page parse budget surfaces as `full_sync_incomplete`, so the known limitation above is now visible in the UI instead of only in daemon logs. Codes never carry page titles, vault paths, or block identifiers; unknown codes are ignored by the client.
...
- **Deterministic Shadow FTS ties** — `search_graph(bm25)` now orders equal BM25 ranks by block UUID, so repeated retrieval produces a stable result order suitable for reusable inference prefixes.
...
- **v2.0 preparation index** — [`docs/roadmaps/ROADMAP_V2_PREPARATION.md`](docs/roadmaps/ROADMAP_V2_PREPARATION.md) (five phases, Safe-Sync, semver rollout); phase issues [#174](https://github.com/MarcoPorcellato/matryca-plumber/issues/174)–[#178](https://github.com/MarcoPorcellato/matryca-plumber/issues/178); slices [#179](https://github.com/MarcoPorcellato/matryca-plumber/issues/179)–[#186](https://github.com/MarcoPorcellato/matryca-plumber/issues/186); [`v2_preparation_blueprints.md`](v2_preparation_blueprints.md); [`docs/openspec/biological-memory.md`](docs/openspec/biological-memory.md); `scripts/populate_v2_preparation.sh`; Epic [#20](https://github.com/MarcoPorcellato/matryca-plumber/issues/20) bodies refreshed.
...
- **LLM structured output** — Pydantic LLM/outline models use `extra="forbid"`; OpenAI strict JSON schemas are generated recursively via the SDK helper; completion calls emit `max_completion_tokens` for o-series/gpt-5 models and legacy `max_tokens` for local servers.
...
- **Lock sidecar permissions** — cross-process flock files (`json_flock`, page RMW locks, daemon lock) created with `0o600` instead of `0o644` (CodeQL `py/overly-permissive-file`).
