...
- **`maintenance_daemon` SRP slice ([#58](https://github.com/MarcoPorcellato/matryca-plumber/issues/58) partial)** — extract `daemon_state.py` (checkpoint ledger load/save) and `daemon_process_lock.py` (PID file, cross-process lock, graceful stop); `maintenance_daemon.py` remains orchestrator with backward-compatible re-exports.
